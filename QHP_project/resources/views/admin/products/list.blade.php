					...
					<table class="user-list" border="1">
						...
						<tbody>
							...
							<tr>
								...
								<td><img src="{{ asset('assets/images/' . $item->HinhAnh) }}" alt="ảnh giày" style="width:50px; height:50px;"></td>
								<td>{{$item->TenTheLoai}}</td>
								<td>{{$item->TenDanhMuc}}</td>
                                <td>
                                    <button class="btn-detail"><a href="{{route('users.edit', ['id'=>$item->MaSP])}}">Chi tiết</a></button>
                                </td>
								<td>
									<button class="btn-update"><a href="{{route('users.edit', ['id'=>$item->MaSP])}}">Sửa</a></button>
								</td>
								<td>
									<button class="btn-del">
										<a onclick="return confirm('Bạn có chắc chắn muốn xóa không?')" 
										href="{{route('users.delete', ['id'=>$item->MaSP])}}">Xóa</a>
									</button>
								</td>
							</tr>
							@endforeach
